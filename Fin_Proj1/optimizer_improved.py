...
max_data = []
def fine_tune():
    global max_alloc, max_sharpe_ratio, max_data
    fine_range = 0.1
    start = max_alloc[0]
    if start > fine_range:
        start -= fine_range
    for a in numpy.arange(start, max_alloc[0] + fine_range, 0.01):
        start = max_alloc[1]
        if start > fine_range:
            start -= fine_range
        for b in numpy.arange(start, max_alloc[1] + fine_range - (a - max_alloc[0]) , 0.01):
            start = max_alloc[2]
            if start > fine_range:
                start -= fine_range
            for c in numpy.arange(start, max_alloc[2] + fine_range - (b - max_alloc[1]), 0.01):
                d = numpy.around(1.0 - a - b - c, 2)
                if d >= 0:
